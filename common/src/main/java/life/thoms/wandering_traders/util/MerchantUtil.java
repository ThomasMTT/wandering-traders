...
public class MerchantUtil {
    ...
    public static MerchantOffers getOffersByClass(AbstractTraderEntity trader) {
        ...
        if (trader instanceof DecorationTraderEntity)  return DecorationTraderUtil.generateOffers();
        if (trader instanceof BookTraderEntity) return BookTraderUtil.generateOffers();
        if (trader instanceof ForestTraderEntity) return ForestTraderUtil.generateOffers();
        if (trader instanceof PotionTraderEntity) return PotionTraderUtil.generateOffers();
        if (trader instanceof AnimalTraderEntity) return AnimalTraderUtil.generateOffers();
        if (trader instanceof ExclusiveTraderEntity) return ExclusiveTraderUtil.generateOffers();
        return null;
    }
    ...
    private static void handleSpawnEggItem(ServerLevel level, SpawnEggItem eggItem) {
        Entity entity = eggItem.getType(new ItemStack(eggItem).getOrCreateTag()).create(level);
        // Disable Hoglin
